...
async def define_reroll_luck_command(tree, servers):
    ...
    async def reroll_luck(interaction: discord.Interaction, amount: int):
        ...
        user_bananas = await get_bananas(str(user_id))

        if user_bananas >= amount:
            await remove_bananas(str(user_id), amount)
            luck_to_add = await random_luck(amount)
            
            await set_luck(str(user_id), luck_to_add)
